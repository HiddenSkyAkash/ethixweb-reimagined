...
function isIndiaWorking(date: Date) {
  const hour = Number(
    new Intl.DateTimeFormat("en-US", { hour: "2-digit", hour12: false, timeZone: INDIA_TZ }).format(date)
  );
  const day = new Intl.DateTimeFormat("en-US", { weekday: "short", timeZone: INDIA_TZ }).format(date);
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className="relative flex h-2 w-2">
          ...
          />
          <span className={`relative inline-flex h-2 w-2 rounded-full ${working ? "bg-emerald-400" : "bg-amber-400"}`} />
        </span>
        <Globe2 className="h-3.5 w-3.5 text-primary" />
        <span className="font-medium tabular-nums">🇮🇳 {india}</span>
        {!sameAsIndia && <span className="text-muted-foreground tabular-nums hidden lg:inline">· You {local}</span>}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-3 w-[320px] rounded-2xl glass-strong shadow-elegant p-4 z-50"
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Zap className="h-4 w-4 text-primary" />
                <span className="text-xs uppercase tracking-widest text-muted-foreground">Global Desk</span>
              </div>
              <span
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="rounded-xl bg-foreground/5 p-3">
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Ethixweb · India</p>
              ...
              <div className="rounded-xl bg-primary/10 border border-primary/20 p-3">
                <p className="text-[10px] uppercase tracking-widest text-primary/80">Your Local Time</p>
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Team check · worldwide</p>
              {ZONES.map((z) => (
                <div
                  key={z.code}
                ...
                >
                  ...
                  <span className="text-xs tabular-nums text-muted-foreground">{formatTime(now, z.tz)}</span>
                </div>
              ))}
            </div>
